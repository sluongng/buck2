...
def _impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    ctx.actions.run(
        cmd_args("python3", script, ctx.attrs.script_args, hidden = write_args),
...
def py_assertion(name: str, exec_compatible_with: list[str] = [], **kwargs):
    ...
    # Don't allow cross-running these tests, since they often invoke
    # executables built for the target platform.
