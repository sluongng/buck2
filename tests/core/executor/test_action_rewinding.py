...
async def test_rewinds_multiple_regular_inputs_evicted_from_remote_cache_mid_build(
    ...
    output = result.get_build_report().output_for_target(CONSUMER_TWO_TARGET)
    assert output.read_text() == CONSUMER_TWO_CONTENT
...
async def test_rewinds_tree_file_input_evicted_from_remote_cache_mid_build(
    ...
    await _restart_with_test_env(
        buck,
        {
            ...
            "BUCK2_TEST_FAIL_RE_DOWNLOAD_DIGESTS_ONCE": TREE_PRODUCER_DIGEST,
        },
    )
    result = await buck.build(
    ...
    output = result.get_build_report().output_for_target(TREE_CONSUMER_TARGET)
    assert output.read_text() == TREE_CONSUMER_CONTENT
    await _assert_remote_actions_ran(
        buck,
...
@buck_test()
@requires_buildbuddy_remote
async def test_rewinds_top_level_output_evicted_from_remote_cache_mid_build(
    buck: Buck,
) -> None:
    await _restart_with_test_env(
        buck,
